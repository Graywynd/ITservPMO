package com.pmo.pmoitserv.Model;
// Generated 27 juil. 2016 12:16:57 by Hibernate Tools 4.3.1


import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Id;
import javax.persistence.JoinColumn;
import javax.persistence.ManyToOne;
import javax.persistence.PrimaryKeyJoinColumn;
import javax.persistence.Table;

/**
 * Etranger generated by hbm2java
 */
@Entity
@Table(name="etranger")
@PrimaryKeyJoinColumn(name="idUtilisateur")
public class Etranger extends Utilisateur  implements java.io.Serializable {


/*     @Id
     @Column(name="idEtranger")
     private int idEtranger;
     
     @ManyToOne
     @JoinColumn(name="utilisateur_idUtilisateur")
     private Utilisateur utilisateur;
*/
    public Etranger() {
    }
/*
    public Etranger(Utilisateur utilisateur) {
       this.utilisateur = utilisateur;
    }
   
    public int getIdEtranger() {
        return this.idEtranger;
    }
    
    public void setIdEtranger(int idEtranger) {
        this.idEtranger = idEtranger;
    }
    public Utilisateur getUtilisateur() {
        return this.utilisateur;
    }
    
    public void setUtilisateur(Utilisateur utilisateur) {
        this.utilisateur = utilisateur;
    }
*/



}


